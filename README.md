...
**_✨curring 기법✨_**을 사용하면 좋을것 같았다 (키워드가 있는지) => (세일인지) => (단독인지) => (품절인지) => **필터링된 상품리스트!**
...
무한스크롤 훅을 만들고 **_✨intersection Observer✨_**로 마지막 엘리먼트가 화면에 렌더링 될 때 다음 페이지를 요청해주었다.<br/>
...
setTimeout으로 직접 구현하지 않고 **_✨useDeferredValue✨_**를 이용하여 검색 키워드를 넘겨주어 이벤트를 최소화해주었다. <br/>
